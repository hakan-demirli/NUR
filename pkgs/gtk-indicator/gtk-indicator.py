...
class GtkIndicator:
    def __init__(self, name):
        self.name = name
        self.socket_dir = os.path.join(XDG_RUNTIME_DIR, "gtk-indicator")
        self.socket_path = os.path.join(self.socket_dir, f"{self.name}.sock")
        self.config_path = os.path.join(
            XDG_CONFIG_HOME, "gtk-indicator", f"{self.name}-config.json"
        )
        self.height = 20
        self.width = 200
...
def setup_logging():
    log_dir = os.path.join(XDG_CACHE_HOME, "gtk-indicator")
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, "gtk-indicator.log")
    logging.basicConfig(
        filename=log_file,
        level=logging.ERROR,
